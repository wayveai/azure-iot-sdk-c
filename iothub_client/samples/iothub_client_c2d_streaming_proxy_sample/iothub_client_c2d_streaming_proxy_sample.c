...
// CAVEAT: This sample is to demonstrate azure IoT client concepts only and is
// not a guide design principles or style Checking of return codes and error
// values shall be omitted for brevity.  Please practice sound engineering
// practices when writing production code.
...
static const char* localHost =
    "127.0.0.1";                  // Address of the local server to connect to.
...
// This is the maximum individual payload size currently supported by Azure IoT
// Device Streaming Gateway. When sending more data, split it it chunks with
// sizes no bigger than the value below.
static const size_t MAX_WEBSOCKET_PAYLOAD_SIZE_IN_BYTES = 65536;

// Functions for socket connection to local service:

static void on_ws_send_frame_complete(
    void* context, WS_SEND_FRAME_RESULT ws_send_frame_result) {
  (void)context;
...
static void on_bytes_received(void* context, const unsigned char* buffer,
                              size_t size) {
  (void)context;
  ...
  if (g_is_uws_client_ready) {
    ...
    while (size > 0) {
      send_size = size <= MAX_WEBSOCKET_PAYLOAD_SIZE_IN_BYTES
                      ? size
                      : MAX_WEBSOCKET_PAYLOAD_SIZE_IN_BYTES;

      if (uws_client_send_frame_async(g_uws_client_handle, 2, buffer, send_size,
                                      true, on_ws_send_frame_complete,
                                      NULL) != 0) {
...
static XIO_HANDLE connect_to_local_service() {
  ...
  (void)xio_open(result, on_io_open_complete, NULL, on_bytes_received, NULL,
                 on_io_error, NULL);
...
static void on_ws_frame_received(void* context, unsigned char frame_type,
...
static UWS_CLIENT_HANDLE create_websocket_client(
    DEVICE_STREAM_C2D_REQUEST* stream_request) {
  ...
  (void)sprintf(auth_header_value, "Bearer %s",
                stream_request->authorization_token);

  // Setting up optional HTTP proxy configuration for connecting to streaming
  // gateway:
  ...
  if (proxy_host != NULL) {
    ...
    tls_io_config.underlying_io_interface =
        http_proxy_io_get_interface_description();
    tls_io_config.underlying_io_parameters = &http_proxy_io_config;

    result =
        uws_client_create_with_io(tlsio_interface, &tls_io_config, host_address,
                                  port, resource_name, &protocols, 1);
  } else {
    result = uws_client_create(host_address, port, resource_name, true,
                               &protocols, 1);
  }
  (void)uws_client_set_request_header(result, "Authorization",
                                      auth_header_value);
  (void)uws_client_open_async(result, on_ws_open_complete, NULL,
                              on_ws_frame_received, NULL, on_ws_peer_closed,
                              NULL, on_ws_error, NULL);
...
static DEVICE_STREAM_C2D_RESPONSE* streamRequestCallback(
    DEVICE_STREAM_C2D_REQUEST* stream_request, void* context) {
  (void)context;
...
static void print_traffic_counters() {
  if (g_lastTrafficPrintTime != INDEFINITE_TIME) {
    ...
    if (current_time != INDEFINITE_TIME) {
      if ((g_incomingByteCount > 0 || g_outgoingByteCount > 0) &&
          difftime(current_time, g_lastTrafficPrintTime) >=
              TRAFFIC_COUNTERS_PRINT_FREQ_IN_SECS) {
        char time_str[128];
        struct tm* current_time_tm = localtime(&current_time);
        strftime(time_str, 128, "%F %T UTC%z", current_time_tm);

        (void)printf(
            "[%s] Network traffic (in bytes) (sent=%d; received=%d)\r\n",
...
static void bad_file(const char* filepath, const char* error_message) {
  fprintf(
      stderr,
      "ERROR: Cannot read connection string from file \"%s\", message: %s\n",
...
int main(void) {
  ...
  if (device_handle == NULL) {
    (void)printf(
        "Failure creating the IotHub device. Hint: Check you connection "
        "string.\r\n");
  } else {
...
#ifdef SET_TRUSTED_CERT_IN_SAMPLES
    // Setting the Trusted Certificate.  This is only necessary on system with
    // without built in certificate stores.
    IoTHubDeviceClient_SetOption(device_handle, OPTION_TRUSTED_CERT,
                                 certificates);
#endif  // SET_TRUSTED_CERT_IN_SAMPLES

#if defined SAMPLE_MQTT || defined SAMPLE_MQTT_WS
    // Setting the auto URL Encoder (recommended for MQTT). Please use this
    // option unless you are URL Encoding inputs yourself. ONLY valid for use
    // with MQTT bool urlEncodeOn = true;
    // IoTHubDeviceClient_SetOption(iothub_ll_handle,
    ...
    if (proxy_host) {
      ...
      if (IoTHubDeviceClient_SetOption(device_handle, OPTION_HTTP_PROXY,
                                       &http_proxy_options) !=
          IOTHUB_CLIENT_OK) {
    ...
    if (IoTHubDeviceClient_SetStreamRequestCallback(
            device_handle, streamRequestCallback, NULL) != IOTHUB_CLIENT_OK) {
